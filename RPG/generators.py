...
def generateWeapon(type=None):
    ...
    weapon=items.Weapon(name, baseAttack,attackType, hands, damageDice)
    return(weapon)

def generateSpecies():
    species = random.choice(characters.speciesList)
    return(species)

def generateArchetype():
    archetype = random.choice(characters.archetypesList)
    return(archetype)
  
def generateCharacter(name, species, archetype):
  character = characters.Character(name,species,archetype)
  return(character)